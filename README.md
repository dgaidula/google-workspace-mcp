...
**Binary files.** All three raw-file tools accept `encoding: "base64"` for binary content (`.docx`, `.xlsx`, `.pdf`, `.pptx`, images). Without it, content is written/read as UTF-8 text, which silently corrupts any bytes that aren't valid UTF-8 — a `.docx` is a ZIP archive, so this fails immediately and often invisibly (the file uploads without error but won't open). With `encoding: "base64"`, content is decoded/encoded through a raw `Buffer`, preserving the bytes exactly. One practical limit: content travels as a JSON string in the tool-call payload, and base64 inflates size by roughly a third, so this is solid for typical documents (résumés, JD PDFs) but isn't the right path for very large binary files.

*Why base64 at all, when Drive's own sync client doesn't need it?* Base64 is only doing work for one specific hop: getting binary bytes from the model's response into a JSON-RPC message that travels over stdio to this shim. JSON-RPC is text-only, so raw bytes can't go in a JSON string as-is. Once the shim decodes that string to a temp file on disk, `gog drive upload` takes it from there as a normal binary upload — no further encoding involved. That's fundamentally different from how Drive's native sync client works: it streams bytes directly off disk in resumable chunks over HTTP, with no JSON wrapper and no size held fully in memory, which is why it scales to gigabyte files and this shim doesn't. This bridge is built for documents — résumés, JDs, cover letters — not general-purpose large file transfer. For anything genuinely large, use `gog drive upload` directly from a terminal; it bypasses the shim and base64 entirely and gets Drive's real chunked-upload behavior.
